<template>
  ...
  >
    <el-table-column label="文件名" prop="file_name" width="250">
      <template #default="scope">
        <el-button v-if="scope.row.file_type==='folder'" link style="color: #000000;" @click="open(scope.row)">
          <span class="fileicon">
          ...
          <h3 class="filename">{{ scope.row.file_name }}</h3>
        </el-button>
    ...
    <el-table-column label="修改日期" prop="modification_time" width="120" />
    <el-table-column label="类型" prop="file_type" width="80">
      <template #default="scope">
        <el-tag type="success" v-if="scope.row.file_type === 'folder'">文件夹</el-tag>
    ...
    <el-table-column align="right">
      ...
      <template #default="scope">
        ...
        <el-button size="small" type="primary" @click="handleDownload(scope.$index, scope.row)" :disabled="scope.row.file_type === 'folder'">下载</el-button>
...
const filterTableData = computed(() =>
    ...
    tableData.data.filter(
        (data) =>
            !search.value ||
            data.file_name.toLowerCase().includes(search.value.toLowerCase())
    )
)

// 删除文件
const handleDelete = (index, row) => {
  delFCB(row.id).then(res => {
    ElNotification({
      title: '文件系统',
